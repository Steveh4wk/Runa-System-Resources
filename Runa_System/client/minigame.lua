...
local WAND_SIZE_X = 0.12
local WAND_SIZE_Y = 0.20
local WAND_OFFSET_X = (WAND_SIZE_X / 2.0) * -1.0
local WAND_OFFSET_Y = (WAND_SIZE_Y / 2.0) * -1.0

local SEGMENT_DRAW_LINE_WIDTH = 0.001   -- Thicker lines for better visibility
...
-- Modified drawWand function to use cursorPosition
local function drawWand()
    DrawSprite(
        TEXTURE_DICT,
        TEXTURES.wand,
        cursorPosition.x + WAND_OFFSET_X,
        cursorPosition.y + WAND_OFFSET_Y,
        WAND_SIZE_X,
        WAND_SIZE_Y,
        0.0,
        255, 255, 255, 255
...
local function loadTextures()
    -- Load YTD file first using the correct path
    RequestStreamedTextureDict('dalgona_textures', true)
    
    local timeout = 0
    while not HasStreamedTextureDictLoaded('dalgona_textures') and timeout < 50 do
        Wait(100)
        timeout = timeout + 1
    end
    
    if HasStreamedTextureDictLoaded('dalgona_textures') then
        debugPrint('dalgona_textures loaded successfully!')
    else
        debugPrint('ERROR: Failed to load dalgona_textures after 5 seconds')
    end
end
...
local function gameLoop(pattern)
    ...
    local DEFAULT_LIFES = (pattern == 'dragon') and 10 or 5
    ...
    while isMinigameActive do
        ...
        if lifes <= math.floor(DEFAULT_LIFES * 0.7) then
            drawCrack('crack2', 1.0)
        end

        if lifes <= math.floor(DEFAULT_LIFES * 0.3) then
            ...
            elseif lifes > 0 and GetGameTimer() >= timeoutAfterFailEndAt then
                ...
                if #completedSegments == #segments then
                    ...
                    SetTimeout(500, function()
                        stopGame()
                        hasSucceed = true
                        debugPrint('Minigame completed with pattern: ' .. pattern)
                    end)
                end
            end
        ...
        if isDrawing then
            ...
            if currentTime - lastHammerTime > 300 then
                PlaySoundFrontend(-1, "hammer", "dalgonagame_audiobank", false)
        ...
        -- Draw failure state overlay
        if lifes > 0 then
            local failurePercent = (DEFAULT_LIFES - lifes) / DEFAULT_LIFES * 100
            
            if failurePercent >= 20 and failurePercent < 50 then
                -- STATO1: Yellow warning (20-50% failure)
                DrawRect(0.95, 0.05, 0.08, 0.08, 255, 255, 0, 180)  -- Yellow background
                DrawRect(0.95, 0.05, 0.07, 0.07, 255, 200, 0, 200)  -- Yellow border
            elseif failurePercent >= 50 and failurePercent < 85 then
                -- STATO2: Orange warning (50-85% failure)
                DrawRect(0.95, 0.05, 0.08, 0.08, 255, 165, 0, 180)  -- Orange background
                DrawRect(0.95, 0.05, 0.07, 0.07, 255, 100, 0, 200)  -- Orange border
            elseif failurePercent >= 85 and failurePercent < 100 then
                -- STATO3: Red critical (85-100% failure)
                DrawRect(0.95, 0.05, 0.08, 0.08, 255, 0, 0, 180)  -- Red background
                DrawRect(0.95, 0.05, 0.07, 0.07, 200, 0, 0, 200)  -- Red border
            end
            ...
            AddTextComponentString("STATO: " .. math.floor(failurePercent) .. "%")
            EndTextCommandDisplayText(0.95, 0.05)
        end
    end

    return hasSucceed and pattern or false
end
...
starSegments = interpolate(starSegments, 6)


local dragonSegments = {
    vec2(0.4489583671093, 0.37685185670853),
    vec2(0.45625001192093, 0.39629629254341),
    vec2(0.44843751192093, 0.41944444179535),
    vec2(0.44531252980232, 0.44259259104729),
    vec2(0.44479170441628, 0.44907405972481),
    vec2(0.43750002980232, 0.4518518447876),
    vec2(0.43854168057442, 0.45555555820465),
    vec2(0.44531252980232, 0.47962963581085),
    vec2(0.43489584326744, 0.48240739107132),
    vec2(0.43229168653488, 0.4879629611969),
    vec2(0.43020835518837, 0.50555557012558),
    vec2(0.43177086114883, 0.52962964773178),
    vec2(0.43593752384186, 0.55000001192093),
    vec2(0.44218751788139, 0.55740737915039),
    vec2(0.45208334922791, 0.56666666269302),
    vec2(0.46250003576279, 0.56944441795349),
    vec2(0.46197918057442, 0.55555552244186),
    vec2(0.46875002980232, 0.55462962388992),
    vec2(0.46406251192093, 0.53888887166977),
    vec2(0.47447919845581, 0.52962964773178),
    vec2(0.47968751192093, 0.53148144483566),
    vec2(0.46041670441628, 0.57962960004807),
    vec2(0.45937502384186, 0.61203700304031),
    vec2(0.47604170441628, 0.63888889551163),
    vec2(0.48697918653488, 0.62314814329147),
    vec2(0.51979172229767, 0.60833334922791),
    vec2(0.52968752384186, 0.61666667461395),
    vec2(0.52395838499069, 0.60555553436279),
    vec2(0.53437501192093, 0.60648149251938),
    vec2(0.53854167461395, 0.60740739107132),
    vec2(0.53385418653488, 0.58333331346512),
    vec2(0.55208337306976, 0.5962963104248),
    vec2(0.54375004768372, 0.56203705072403),
    vec2(0.55625003576279, 0.5722222328186),
    vec2(0.54843753576279, 0.53148144483566),
    vec2(0.56979167461395, 0.53703701496124),
    vec2(0.55572921037674, 0.50092589855194),
    vec2(0.57812505960464, 0.49537035822868),
    vec2(0.55677086114883, 0.47222220897675),
    vec2(0.57604169845581, 0.46574074029922),
    vec2(0.56510418653488, 0.44351851940155),
    vec2(0.54479169845581, 0.43703702092171),
    vec2(0.5546875, 0.42685183882713),
    vec2(0.5703125, 0.42499998211861),
    vec2(0.55416667461395, 0.40925925970078),
    vec2(0.53750002384186, 0.40462961792946),
    vec2(0.53020834922791, 0.40925925970078),
    vec2(0.52604168653488, 0.40000000596046),
    vec2(0.54010421037674, 0.38055554032326),
    vec2(0.54218751192093, 0.36574074625969),
    vec2(0.52656251192093, 0.37129628658295),
    vec2(0.51406252384186, 0.39444443583488),
    vec2(0.50833338499069, 0.39722222089767),
    vec2(0.51510417461395, 0.37962961196899),
    vec2(0.51250004768372, 0.37129628658295),
    vec2(0.49114584922791, 0.39074072241783),
    vec2(0.48750001192093, 0.37592592835426),
    vec2(0.47395834326744, 0.39444443583488),
    vec2(0.47135418653488, 0.39537036418915),
    vec2(0.46406251192093, 0.38518518209457),
    vec2(0.45364585518837, 0.37129628658295),
}
dragonSegments = interpolate(dragonSegments, 7)

local squareSegments = {
...
squareSegments = interpolate(squareSegments, 5)
...
triangleSegments = interpolate(triangleSegments, 5)
...
function startMinigame(pattern, cbTrue)
    debugPrint('Starting minigame with pattern: ' .. tostring(pattern))
    ...
    loadTextures()
    if not RequestAmbientAudioBank(AudioBankName, false) then 
    ...
    if not DEBUG_MODE_ADD_NEW_DRAW then
        ...
        -- In debug mode, load the pattern as background for tracing
        segments = {} -- Start with empty segments for manual drawing
    end
...
function stopGame()
    isMinigameActive = false
    SetStreamedTextureDictAsNoLongerNeeded('dalgona_textures')
end

-- Test command
RegisterCommand('dalgona', function(source, args)
    debugPrint('Dalgona command executed with args: ' .. json.encode(args))
    ...
    -- Debug mode: show coordinates for pattern
    if pattern == 'dragon' then
        print('-------- Dragon Pattern Coordinates --------')
        for k, v in ipairs(dragonSegments) do
            print(('vec2(%s, %s),'):format(v.x, v.y))
        end
        print('------------------------------------------')
        Framework.showNotification('Dragon coordinates printed to console')
        Framework.showNotification('Starting debug mode with dragon pattern background')
        startMinigame('dragon') -- Start minigame with dragon pattern as background
        return
    end
...
function startRoulette(targetTexture)
    if not targetTexture then
        finalTexture = patternsList[math.random(#patternsList)]
    else
...
RegisterNetEvent(EVENTS['gameStarted'], function()
    ...
    -- Init mini-game
    local result = startMinigame(pattern, function()
        return gameStarted
    end)
    -- Set status of mini-game
    LocalPlayer.state:set(STATEBAGS['minigameSucceed'], result ~= false, true)
end)

-- Debug print function
local function debugPrint(msg)
    print('[Dalgona] ^3' .. tostring(msg) .. '^7')
end

RegisterNetEvent('dalgona:startMinigame', function()
    debugPrint('Starting Dalgona minigame...')
    local result = startMinigame(nil)
    debugPrint('startMinigame returned: ' .. tostring(result))
    if result then
        debugPrint('Minigame WON with pattern: ' .. tostring(result) .. ', triggering server event...')
        TriggerServerEvent('dalgona:giveReward', result)
    else
        debugPrint('Minigame LOST')
    end
end)
